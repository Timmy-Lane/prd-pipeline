...
7. **Template-section coverage** — every section the active template marks **required** (bold) is present AND non-empty. A missing or stub required section (e.g. **Wedge**, **Alternatives considered**, **Drawbacks/invalidators**) is **CRITICAL**. Don't leave this to interpretation under another pass — check the literal section set.

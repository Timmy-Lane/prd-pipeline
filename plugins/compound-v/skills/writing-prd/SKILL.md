...
- This skill owns the **document quality** of any PRD *or* feature spec — sections present, alternatives weighed, invalidators measurable, claims sourced, prose lean. The per-build *workflow* that drives a feature spec to shipped code (tier routing, adversarial grill, the plan-gate, the parallel worktree build) is **prd-pipeline**, which *composes* this skill — don't re-implement that workflow here.
...
- **Goal** — what the product is, who it's for, and the one outcome it exists to deliver — framed working-backwards from the customer (the outcome in their words, one paragraph). Plain language, not a mission statement.
...
- **Tech stack & architecture** — the languages, frameworks, and services it runs on, a one-sentence shape of the system, the load-bearing design decisions *and the alternatives weighed against them* (a design with no rejected alternative wasn't designed, just transcribed), and the cross-cutting constraints that shape everything downstream (security/privacy boundary, cost envelope, observability, data retention). The slow-changing *what* and *why* — never the per-build *how*.
- **Key bets & invalidators** — the load-bearing assumptions the product is wagered on, each as a triple: the observable condition, how you'd measure it, and what a break would mean. A bet you'll "revisit later" must carry a concrete trigger — N users/runs, a named event, a data threshold — **never calendar time** ("in two weeks"), which is unmeasurable and rots. (Per-build risks stay in the plan; these are the durable product-level bets.)
...
## Writing discipline
Prose where precision matters — writing the sentence forces the thinking a bullet lets you skip. Cite or tag every load-bearing claim (`[ASSUMPTION]` when you're guessing); an unsourced magic number is a defect. Encode only what transfers across model generations. Before you call it done, self-check that every section above is present and non-empty — an empty *Non-goals* or a missing *alternatives* means it wasn't finished, not that it didn't apply. Keep it lean: if `CLAUDE.md` links it, you pay for it at every session start.

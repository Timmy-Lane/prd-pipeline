...
> Self-driven targeted research, 2026-05-30; §8–§9 added 2026-07-28 after the skill was observed
> producing more documents than readers. Sources cited inline; full list at bottom. This is
> reference material, not the skill itself.
...
## 8. Document proliferation — the failure this pipeline had, and the fix

**The observed defect.** Running the pipeline as originally written, a single T2 feature produced
three permanent files: the spec, a sibling `<spec>-analysis.md` from the consistency gate, and a
plan under `docs/plans/`. None was ever deleted (specs are append-only), none was folded back into
anything, and nothing in the procedure ever *read* the project's existing documentation before
deciding to write more. The result is the classic spec-driven failure: a growing pile of
overlapping change records, and a README that quietly contradicts all of them.

**Prior art that solves it: OpenSpec's persistent/ephemeral split.** OpenSpec separates a
`specs/` folder of *cumulative requirements* — "persistent requirements that define what the system
should do … the source of truth" — from a `changes/` folder of *temporary artifacts*: proposal,
design, tasks, "that eventually gets archived." On completion, `archive` moves the change folder
to `changes/archive/<timestamp-name>/` and the specs absorb what was learned. The stated principle:
specs are "cumulative requirements that evolve, not proliferate," and "the proposal, design, and
tasks are temporary scaffolding; only refined requirements graduate into the persistent specs
layer." [OpenSpec README, fetched 2026-07-28]

**Prior art on the other side: spec-kit's per-feature directory.** spec-kit gives every feature its
own numbered directory — `spec.md`, `plan.md`, `research.md`, `data-model.md`, `contracts/`,
`quickstart.md`, `tasks.md` — seven artifacts per feature, kept in sync because "plans and tasks
derive from these documents systematically." Its durable, cross-feature layer is a single
`memory/constitution.md`, "the architectural DNA of the system," which participates in compliance
checks for every later spec. [spec-kit `spec-driven.md`, fetched 2026-07-28]

Both are internally coherent, and they answer different questions. spec-kit's fan-out is right when
many machine agents consume typed artifacts. For a human-plus-agent repo it is a maintenance tax
paid every feature. **The design adopted here takes OpenSpec's split and spec-kit's constitution
idea, and drops the fan-out:**

| Decision | Rule in SKILL.md |
|---|---|
| Route before writing | Step 1.5 inventories the existing docs and returns one of *amend · supersede · create · none* |
| Cap the output | Artifact budget: one change ⇒ at most one new document, T2 only |
| No siblings | Consistency findings and the plan are **sections** of the change file |
| Nothing for the small tiers | T0 and T1 produce zero documents; the record is the commit message |
| Nothing rots silently | `stale_docs` in the frontmatter → plan tasks → the Step 6.4 doc-sync gate |
| Scaffolding does not accumulate | At ship, the durable decision folds into the living spec or an ADR; the change file is archived |

**Why "amend" is the default and not "create".** A change record answers *what one change did*; a
living spec answers *what the system does now*. Only the second is worth keeping indefinitely, and
it is worth exactly one copy. Step 2.5 pass 5 therefore treats "an existing document already covers
≥60% of this surface" as a CRITICAL routing error rather than a style note — because by the time a
reviewer notices, the duplicate is already committed.

**Documentation modes.** Diátaxis is the working taxonomy for *which* document a fact belongs in —
tutorial (learning), how-to (task), reference (information), explanation (understanding) — chosen by
the reader's intent at the moment they arrive. Step 1.5's five buckets are that taxonomy applied to
a code repo: product intent (explanation), decisions/ADR (explanation), change records
(explanation, time-boxed), runbooks (how-to), API/CLI/config (reference). The practical consequence
is the doc-sync gate: a behavior change almost always invalidates a *reference* doc and sometimes a
*how-to*, and those are the two modes nobody remembers to update because neither is where the
thinking happened. [diataxis.fr]

## 9. Stage verdicts must live on disk, not in the window

Compaction re-attaches only the most recent invocation of each skill, under a shared budget. A long
T2 run — taste → spec → consistency gate → grill → plan → build → verify → ship — therefore loses
its own early verdicts by the time it is building: which alternative was rejected, what the grill
blocked on, what exactly the human approved. Recovering *the phase* from a todo list is not the
same as recovering *the decisions*.

The fix is the same discipline a research vault already uses — the artifact is on disk, not in the
window: an append-only `.claude/prd-pipeline/<change-id>.jsonl`, one line per stage, never
rewritten (a changed decision is a new line that supersedes). Append-only and one-line-per-stage
are both load-bearing: a log that grows into a report gets evicted for exactly the reason the skill
body did. Prior art for the shape: OpenHands SDK v1 splits mutable conversation metadata from an
append-only `EventLog` so persistence is incremental and crash recovery is cheap.

## Sources
- OpenSpec (Fission-AI) — https://github.com/Fission-AI/OpenSpec — persistent `specs/` vs ephemeral `changes/`, archive-on-complete
- GitHub spec-kit `spec-driven.md` — https://github.com/github/spec-kit/blob/main/spec-driven.md — per-feature artifact set, `memory/constitution.md`
- Diátaxis — https://diataxis.fr/ — the four documentation modes and choosing by reader intent

...
- fast → `Skill(skill: "bad-research:bad-research-fast")` (bounded-ReAct = a step-capped Reason+Act loop; replaces 2–14)
- ultrafast → `Skill(skill: "bad-research:bad-research-ultrafast")` (commercial-DR middle tier — plan → K parallel researchers → leader synthesis; replaces 2–14; explicit `--ultrafast`/"ultrafast mode" only, fully autonomous — skips 0.5 + 1.6)

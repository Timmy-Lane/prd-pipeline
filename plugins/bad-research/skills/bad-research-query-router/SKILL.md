...
- **fast** → `Skill(skill: "bad-research:bad-research-fast")` (then the slim citation-grounding pass + slim critic before step 15 polish)
- **ultrafast** → `Skill(skill: "bad-research:bad-research-ultrafast")` (commercial-DR middle tier — plan → K parallel researchers → leader synthesis; then the same slim citation-grounding + slim critic tail as fast)

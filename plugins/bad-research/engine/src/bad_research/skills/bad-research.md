...
0. **Auto-init if missing.** Two checks for the first-run-after-global-install case:
   ...
   - **Step-skills check (lazy install — standalone installs ONLY).** **First check whether the step skills are already present as `bad-research:bad-research-N-...` in this session's skill listing.** If they are, you are running the bundled plugin, which ships every step skill: do NOT run `bad install --steps-only`, because it would spray a second, diverging copy of all 19 step-skill directories into the working repo, and nothing ever prunes them. Only when the namespaced skills are absent AND `.claude/skills/bad-research-1-decompose/SKILL.md` does not exist relative to the working directory, run `bad install --steps-only . --json` — the standalone user-global install ships only the entry skill + agents + PreToolUse hook, so the step skills have to materialize per-project.
